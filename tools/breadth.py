...
**مرزِ هفته حالا برای هر نماد جدا از داده درمی‌آید** (`calendar_wk.py`).
قبلاً لنگرِ شنبه روی همه اعمال می‌شد و همین باگی بود که مصطفی روی دلار
گرفت: اونس طلا و نقره و نفت دوشنبه تا جمعه‌اند، نه شنبه تا چهارشنبه.
"""
import argparse
import json
import statistics
...
from calendar_wk import derive_anchor, week_key_for
...
def box_states(rows, kind, anchor=None):
    """(حالت ماهانه، حالت هفتگی، ریسک ماهانه، ریسک هفتگی) یا None.

    `anchor` روزِ شروعِ هفته است (۰=دوشنبه … ۵=شنبه). `None` یعنی از خودِ
    داده دربیاور — برای صندوق‌های بورسی شنبه درمی‌آید و برای محرک‌های
    جهانی دوشنبه. صندوق‌ها پنجشنبه و جمعه کندل ندارند پس لنگرشان عوض
    نمی‌شود؛ این پارامتر فقط محرک‌ها را درست می‌کند.
    """
    ...
    return {"close": close, "date": rows[-1][0],
            ...
            # گیتِ ۲۰کندلی که دیروز گذاشته بودم **باطل شد**. تشخیصم غلط
            # بود: `tools/h1_box.py` روی سه نمادی که H1 دارند نشان داد
            # H1 و روزانه در ۹۲٪ هفته‌ها یک حالت می‌دهند و پهنای باکس
            # تقریباً یکی است (۰٫۷۸٪ در برابر ۰٫۸۰٪). مقصرِ باگِ دلار
