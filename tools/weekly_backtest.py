...
# مرزِ هفتهٔ صندوق‌های بورسی: **یکشنبه تا شنبه** — یعنی کندلِ شنبه
# آخرین کندلِ باکس است، نه اولین کندلِ هفتهٔ بعد.
#
# این با بند ۰ قانون ۱ («هفته شنبه تا چهارشنبه») فرق دارد و عمدی است.
# آن قانون **تقویمِ معاملاتی** را درست توصیف می‌کند (بازار شنبه باز
# می‌شود و چهارشنبه می‌بندد، و شمارشِ داده هم همین را داد: ۸۲٪).
# ولی **پنجرهٔ باکس** یک چیز دیگر است، و مصطفی آن را این‌طور تعریف کرد:
# «پایان دورهٔ باکس شنبه، روز تصمیم یکشنبه، روز اجرا دوشنبه.»
#
# اندازه گرفته شد (`tools/decision_day.py`، مزیتِ درون‌هفته‌ای، تصمیم
# روی کلوزِ یکشنبه در هر دو حالت):
#
#     باکس شنبه..چهارشنبه (شنبهٔ تازه بیرون)   +۰٫۳۸۸ واحد  t=۰٫۹۶  ۳۶ هفته
#     باکس یکشنبه..شنبه   (شنبه داخل)          +۰٫۶۵۲ واحد  t=۱٫۵۱  ۳۹ هفته
#
# ۶۸٪ بهتر. ⚠️ t=۱٫۵۱ بزرگ نیست و +۰٫۲۶ واحدِ اختلاف از خطای نمونه جدا
# نمی‌شود؛ ولی هم بهتر اندازه گرفت و هم قاعدهٔ خودِ اوست.
WEEK_ANCHOR = 6          # یکشنبه


def week_key(d):
    """کلیدِ هفته = تاریخِ یکشنبهٔ همان هفته (هفته یکشنبه→شنبه)."""
    back = (d.weekday() - WEEK_ANCHOR) % 7
    sun = d - timedelta(days=back)
    # load_daily گاهی datetime می‌دهد گاهی date؛ کلید باید یک جنس باشد
    return sun.date() if hasattr(sun, "date") else sun
    ...
    for _, b in fwd:
        if not entered:
            if b.l <= hi:
                entered = True
                if b.l <= lo:
                    # ورود و استاپ در یک کندل — تفکیک‌ناپذیر با دادهٔ روزانه
                    return ("استاپ‌هم‌کندل", -1.0)
                if b.h >= target:
                    return ("تارگت", 1.0)
            continue
        if b.l <= lo:
...
def main():
        ...
        for i in range(len(ws) - 1):
            ...
            for k in kinds:
                ...
                per_kind[k].append({"w": b, "sym": name,
                                    "st": st, "ret": ret})
                # هندسه فقط روی هفته‌هایی که **سیگنال داده‌اند**. اگر روی همهٔ
                # هفته‌ها اجرا شود، هفته‌هایی که قیمت از اول داخل یا زیر باکس
                # بوده هم می‌آیند — آنجا ورود و استاپ بدیهتاً هم‌زمان‌اند و
                # ستون «هم‌کندل» مصنوعی بالا می‌رود. بک‌تست ماهانه همین قید
                # را دارد (`if st == "بالا"`).
                if st == "بالا":
    ...
    print(f"  {'تعریف':<16}{'سیگنال':>8}{'خالی':>8}{'تارگت':>8}"
          f"{'استاپ':>8}{'هم‌کندل':>8}{'سهم':>7}{'میانگین R':>11}")
    geom_out = {}
    for k in kinds:
        g = geom[k]
        if not g:
        ...
        amb = sum(1 for o, _ in g if o == "استاپ‌هم‌کندل")
        avg = statistics.mean(rs) if rs else float("nan")
        geom_out[k] = {"n": len(g), "empty": e / len(g) * 100, "target": tp,
                       "stop": sl, "ambiguous": amb,
                       "ambiguous_pct": amb / len(g) * 100, "avg_r": avg}
        print(f"  {k:<16}{len(g):>8}{e/len(g)*100:>7.0f}٪{tp:>8}{sl:>8}"
              f"{amb:>8}{amb/len(g)*100:>6.0f}٪{avg:>+11.3f}")
    worst = max((v["ambiguous_pct"] for v in geom_out.values()), default=0)
    print(f"\n  ستون «هم‌کندل»: ورود و استاپ در یک کندل افتاده‌اند. باکس")
    print("  باریک‌تر از دامنهٔ یک روز است، پس دادهٔ روزانه نمی‌داند اول کدام")
    print(f"  رخ داده. بیشترین سهم اینجا {worst:.0f}٪ است"
          + (" — قابل چشم‌پوشی نیست، ولی"
             if worst < 20 else " — جدول را با احتیاط بخوانید؛"))
    print("  ستون «مزیت» بالا از این مشکل مصون است، چون بازده را کلوز به")
    print("  کلوز می‌گیرد و اصلاً به مسیرِ داخلِ دوره کار ندارد.")
    print("\n  بند ۳ CLAUDE.md همین قاعده را ۷۰٫۲٪ و +۰٫۴۰۹R ثبت کرده")
    print("  (n=۴۲۹، ۱۱ از ۱۲ نماد). دقت کنید کدام سطر بالا به آن نزدیک است.")
