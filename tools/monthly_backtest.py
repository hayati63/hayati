...
def geometry_trade(box, fwd_bars):
    ...
    'خالی' / 'تارگت' / 'استاپ' / 'استاپ‌هم‌کندل' / 'پایان ماه'

    ⚠️ **حدِ تفکیک.** استاپ زیر کف باکس است. اگر باکس از دامنهٔ یک کندل
    باریک‌تر باشد، ورود و استاپ در یک کندل می‌افتند و دادهٔ روزانه نمی‌داند
    اول کدام رخ داده. چنین حالتی جداگانه برچسب می‌خورد تا با استاپِ واقعی
    قاطی نشود — نتیجهٔ قاعده نیست، حدِ تفکیکِ داده است.
    """
    lo, hi = box
    height = hi - lo
    if height <= 0:
    ...
    for _, b in fwd_bars:
        if not entered:
            if b.l <= hi:                      # پولبک خورد
                entered = True
                # همان کندل ممکن است استاپ هم بخورد. بدبینانه اول استاپ،
                # ولی با برچسب جدا چون تفکیک‌ناپذیر است.
                if b.l <= lo:
...
def main():
    ...
    print(f"  {'تعریف':<16} {'سیگنال':>7} {'خالی':>7} {'تارگت':>7}"
          f" {'استاپ':>7} {'هم‌کندل':>7} {'سهم':>6} {'میانگین R':>10}")
    for k in kinds:
        g = geom[k]
        if not g:
            continue
        ...
        print(f"  {k:<16} {len(g):>7} {empty/len(g)*100:>6.0f}٪"
              f" {tp:>7} {sl:>7} {amb:>7} {amb/len(g)*100:>5.0f}٪"
              f" {avg:>+10.3f}")
    print("  «خالی» = حمایت خالی: پولبک نخورد، پس ورودی نبود.")
    print("\n  ⚠️ ستون «هم‌کندل»: ورود و استاپ در یک کندل افتاده‌اند. دادهٔ")
    print("  روزانه نمی‌داند اول کدام رخ داده، پس اینها **اندازه‌گیری**")
    print("  نیستند. هرچه این ستون بزرگ‌تر، «میانگین R» بی‌معناتر.")
    print("  برای تفکیکشان کندلِ ریزتر لازم است — H4 برای ماهانه، H1 برای")
    print("  هفتگی. همان چیزی که بند ۱ CLAUDE.md می‌گوید.")
    ...
    if args.json_out:
        ...
        for k in kinds:
            ...
            geom_out[k] = {
                ...
                "ambiguous": amb, "ambiguous_pct": amb / len(g) * 100,
